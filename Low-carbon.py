...
with col1:
    strength_MPa = st.number_input("f'c ที่ 28 วัน (MPa)", value=25.0, min_value=0.0, step=0.5)
...
if strength_MPa < 15:
    st.warning("⚠️ f'c < 15 MPa — ได้ 0 คะแนนหมวดกำลังอัด")
elif strength_MPa < 18:
    st.info(f"f'c = {strength_MPa:.1f} MPa (15–17 MPa = 8 คะแนน)")
else:
    st.success(f"✅ f'c = {strength_MPa:.1f} MPa")
...
with st.expander("📖 ตารางอ้างอิงเกณฑ์การให้คะแนน"):
    st.write("#### หมวด 1: กำลังอัดคอนกรีต (35 คะแนน) — f'c ที่ 28 วัน")
    st.write("เกณฑ์ขั้นต่ำ: f'c ≥ 15 MPa (ไม่ตัดสิทธิ์ แต่ < 15 MPa ได้ 0 คะแนน)")
    df_s = pd.DataFrame({
        "f'c ที่ 28 วัน": ["≥ 30 MPa", "27–29 MPa", "24–26 MPa", "21–23 MPa", "18–20 MPa", "15–17 MPa", "< 15 MPa"],
